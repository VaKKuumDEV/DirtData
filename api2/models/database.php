...
class Database{
	...
	public function __construct(){
		...
		$this->db = new mysqli("localhost", "remote", "Q4ZsiCuPhokQfKgx4m6jIiMirDNMFVXH", "krem");
	...
	public function updateData(string $table, array $data, array $where): bool{
		...
		foreach($where as $key => $value){
			if(is_string($value)) $value = "'" . $this->real_string($value) . "'";
			$joinedWhere[] = $key . ' = ' . $value;
		}
		
		$sql = "UPDATE " . $table . " SET " . implode(', ', $joinedData) . " WHERE " . implode(', ', $joinedWhere);
		$result = $this->db->query($sql);
		return $result;
	}
